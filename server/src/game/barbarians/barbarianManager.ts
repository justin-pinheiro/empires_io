...
export class BarbarianManager {
  ...
  constructor(private state: GameState, private game: GameEngine) {
    if (!this.state.getPlayer(this.playerId)) {
      this.state.addPlayer(this.playerId);
    }
  }
  ...
  private spawnCamps() {
    ...
    this.state.addResourcesToPlayer(this.playerId, new Resources(0, 0, 0, 0, totalSpawns * BARBARIAN_TROOPS_PER_CAMP));

    for (const id of players) {
      const tileKeys = this.state.getMap().getAllTileKeys();
      const playerOwnedBuildings = tileKeys.filter(t => this.state.getBuilding(t)?.getOwnerId() === id);
      const neighborCandidates = new Set<string>();
      playerOwnedBuildings.forEach(t => {
        const neighbors = this.state.getMap().getNeighborsIds(t, 1);
        neighbors.forEach(n => {
          if (!this.state.getBuilding(n)) neighborCandidates.add(n);
        });
      });
      ...
      for (let i = 0; i < numToSpawn; i++) {
        ...
        if (tileId && !this.state.getBuilding(tileId)) {
          this.state.addBuilding(this.playerId, 'BARBARIAN_CAMP', tileId);
  ...
  private performAttacks() {
    for (const tileId of Array.from(this.campTileIds)) {
      const neighbors = this.state.getMap().getNeighborsIds(tileId, 1);
      const possibleTargets = neighbors.filter(n => {
        const b = this.state.getBuilding(n);
      ...
      if (!this.state.getBuilding(tileId) || this.state.getBuilding(tileId)?.getOwnerId() !== this.playerId) {
